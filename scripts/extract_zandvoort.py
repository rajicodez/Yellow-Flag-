...
def extract_track(image_path, output_svg_path, output_img_path):
    ...
    # Extract Red Line (BGR format in OpenCV)
    # R > 180, G < 100, B < 100, (R - G) > 110
    # Also ensuring we avoid orange (where G is often > 100)
    ...
    if len(pts_red) > 0:
        print(f"Red mask bounds: minX={np.min(pts_red[:, 1])}, maxX={np.max(pts_red[:, 1])}, minY={np.min(pts_red[:, 0])}, maxY={np.max(pts_red[:, 0])}")


    print("Using minimal closing to preserve parallel tracks...")
    kernel_size = 3
    ...
    # Join endpoints of disconnected components
    ...
    while True:
        ...
        # Find the pair of endpoints in DIFFERENT components that are closest
        best_pair = None
        min_dist = float('inf')
    ...
    # Using light smoothing to preserve the tight corners
    ...
    # Enforce Clockwise (Zandvoort runs clockwise)
    ...
    # Because we don't manually pick the start index here, the game uses totalProgress=0.
    # In F1Racer, start position is driven by `progress=0`. In SVG, M is index 0.
    # We should shift the array so that index 0 is at the bottom horizontal straight.
    # The bottom straight has high Y and is in the middle of X.
    bottom_pts = np.argwhere(final_pts[:, 1] > (viewbox_h * 0.8)).flatten()
    if len(bottom_pts) > 0:
        # Find the point on the bottom straight that is roughly in the middle x
        mid_x = viewbox_w / 2
    ...
    overlay = img.copy()
    # Draw original path as bright cyan over the native image
    for i in range(len(ordered_pts)):
        p1 = tuple(ordered_pts[i].astype(int))
        p2 = tuple(ordered_pts[(i+1)%len(ordered_pts)].astype(int))
        cv2.line(overlay, p1, p2, (255, 255, 0), 2) # Cyan in BGR is (255, 255, 0)
        
    cv2.imwrite(output_img_path, overlay)
    print("Extraction successful.")
    
if __name__ == '__main__':
    img_path = r"C:\Users\ASUS\.gemini\antigravity\brain\e94b5422-e6bd-4b92-a1e9-399841580f0f\.user_uploaded\media__1784894606590.png"
    out_svg = r"w:\sinhala-f1-podcast\src\assets\tracks\zandvoort.svg"
    out_img = r"C:\Users\ASUS\.gemini\antigravity\brain\e94b5422-e6bd-4b92-a1e9-399841580f0f\zandvoort_overlay.jpg"
