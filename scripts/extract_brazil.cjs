...
async function extract() {
    ...
    // 1. Threshold
    console.log("Thresholding dark pixels...");
    for(let y=0; y<h; y++){
        for(let x=0; x<w; x++){
            const idx = (y*w + x) * 4;
    ...
    let startIdx = -1;
    // Try to find the start/finish line roughly at left-middle
    let bestStartDist = Infinity;
    for (let i = 0; i < w * h; i++) {
        if (grid[i]) {
            let cx = i % w;
            let cy = Math.floor(i / w);
            // distance from x=100, y=250
            let d = Math.hypot(cx - 100, cy - 250);
            if (d < bestStartDist) {
                bestStartDist = d;
                startIdx = i;
            }
  ...
  <path d="${d}" />
    ...
    fs.writeFileSync('src/assets/tracks/brazil.svg', svgData);
    console.log("Wrote src/assets/tracks/brazil.svg");
    ...
    fs.writeFileSync('public/debug_brazil.json', JSON.stringify(outData, null, 2));
    console.log("Wrote public/debug_brazil.json");
}
