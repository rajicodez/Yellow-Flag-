...
def extract_track(image_path, output_svg_path, output_img_path):
    ...
    # COTA: Extract pure red corridor
    red_mask = (
        (r > 150) & 
        (r > g * 1.5) & 
        (r > b * 1.5) & 
    ...
    # Bridge small gaps in the red stroke
    print("Bridging small gaps...")
    ...
    if num_labels > 1:
        ...
        fused_mask = (labels == largest_label).astype(np.uint8) * 255
    ...
    window = 5 # Moderate smoothing for COTA's fast curves
    ...
    # COTA's start/finish line is on the long diagonal straight on the far left.
    # The start/finish straight runs from bottom-middle/left (T20) to bottom-center (T1).
    # We want to find the point on this straight.
    # In the image, T20 is on the far left. T1 is at the bottom.
    # The straight connects them.
    # We can look for points with X < viewbox_w * 0.4 and Y > viewbox_h * 0.6.
    start_pts = np.argwhere((final_pts[:, 0] < (viewbox_w * 0.4)) & (final_pts[:, 1] > (viewbox_h * 0.6))).flatten()
    if len(start_pts) > 0:
        # Align roughly in the middle of this cluster
        best_idx = start_pts[len(start_pts) // 2]
...
if __name__ == '__main__':
    # Assuming media__1784903929175.jpg is the red one. If not, it might be the other one.
    img_path = r"C:\Users\ASUS\.gemini\antigravity\brain\e94b5422-e6bd-4b92-a1e9-399841580f0f\.user_uploaded\media__1784903929175.jpg"
    out_svg = r"w:\sinhala-f1-podcast\src\assets\tracks\cota.svg"
    out_img = r"C:\Users\ASUS\.gemini\antigravity\brain\e94b5422-e6bd-4b92-a1e9-399841580f0f\cota_overlay.jpg"
