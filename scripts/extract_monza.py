...
def extract_track(image_path, output_svg_path, output_img_path):
    ...
    # Monza: The road is black. We ignore the grey shadow, white background, orange text, and turquoise circles.
    # We will use the region roughly corresponding to the track bounds: X=227-1052, Y=120-535.
    ...
    # Restrict to approximate track region to ignore black text elsewhere
    roi_mask = np.zeros_like(black_mask)
    roi_mask[100:555, 200:1072] = 255
    black_mask = cv2.bitwise_and(black_mask, roi_mask)
    
    pts_black = np.argwhere(black_mask > 0)
    print(f"Black pixels found: {len(pts_black)}")
    if len(pts_black) > 0:
        print(f"Mask bounds: minX={np.min(pts_black[:, 1])}, maxX={np.max(pts_black[:, 1])}, minY={np.min(pts_black[:, 0])}, maxY={np.max(pts_black[:, 0])}")

    # Keep only the largest connected component (the road itself)
    ...
    print("Bridging small gaps (checkered flag, turquoise markers)...")
    # Apply morphological closing with a large kernel.
    # Monza has no tight parallel tracks, so a large kernel is safe.
    kernel_size = 25
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    fused_mask = cv2.morphologyEx(black_mask, cv2.MORPH_CLOSE, kernel)
    
    # Fill any holes inside the thick road
    contours, hierarchy = cv2.findContours(fused_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    for i, contour in enumerate(contours):
        if hierarchy[0][i][3] != -1: # Inner contour (hole)
            cv2.drawContours(fused_mask, [contour], 0, 255, -1)
    ...
    # The checkered flag is on the bottom horizontal straight, moving from left to right.
    # The bottom straight has high Y and is in the middle-right of X (before Prima Variante).
    # Prima Variante is around X=600-700 on the bottom. Let's find a point on the bottom straight.
    bottom_pts = np.argwhere(final_pts[:, 1] > (viewbox_h * 0.8)).flatten()
    if len(bottom_pts) > 0:
        # Start/finish is on the straight, maybe mid_x of the straight.
        # Let's just use the rightmost part of the bottom straight before the turn.
        mid_x = viewbox_w * 0.6
        best_idx = bottom_pts[np.argmin(np.abs(final_pts[bottom_pts, 0] - mid_x))]
...
if __name__ == '__main__':
    img_path = r"C:\Users\ASUS\.gemini\antigravity\brain\e94b5422-e6bd-4b92-a1e9-399841580f0f\.user_uploaded\media__1784898353771.jpg"
    out_svg = r"w:\sinhala-f1-podcast\src\assets\tracks\monza.svg"
    out_img = r"C:\Users\ASUS\.gemini\antigravity\brain\e94b5422-e6bd-4b92-a1e9-399841580f0f\monza_overlay.jpg"
