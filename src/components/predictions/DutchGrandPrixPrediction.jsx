...
export default function DutchGrandPrixPrediction() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (parsed.status === 'submitted') {
        if (new Date() > new Date(parsed.closeTime)) {
          setIsClosed(true);
          setStage('review');
        } else {
  ...
  const validateP1P2P3 = () => {
    const p1 = answers[1]; // POLE
    const p2 = answers[3]; // P2
    const p3 = answers[4]; // P3
    const winner = answers[2]; // RACE WINNER

    // Since questions 2,3,4 are P1,P2,P3 for the race:
    // Let's check uniqueness of Race Winner (2), P2 (3), P3 (4) if they are answered
    
    // We only validate the current question if it's one of these
    const currentQId = dutchGPQuestions[currentQuestionIndex].id;
    
    if ([2, 3, 4].includes(currentQId)) {
      const selectedDrivers = [winner, p2, p3].filter(Boolean);
      const uniqueDrivers = new Set(selectedDrivers);
      
      if (selectedDrivers.length !== uniqueDrivers.size) {
  ...
  const handleSubmit = () => {
    // Final check
    const isComplete = dutchGPQuestions.every(q => answers[q.id]);
    if (!isComplete) {
