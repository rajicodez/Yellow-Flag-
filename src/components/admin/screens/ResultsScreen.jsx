...
import { constructorOptions, driverOptions, dutchGrandPrixQuestions } from '../data';
    ...
    <div className="space-y-7">
      ...
      <Panel className="p-5 sm:p-6">
        <div className="grid gap-5 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-end">
          <label className="block text-sm font-bold text-zinc-200"><span className="mb-2 block">Race</span><select className={inputClass} defaultValue="Dutch Grand Prix"><option>Dutch Grand Prix</option><option>Hungarian Grand Prix</option><option>British Grand Prix</option></select></label>
      ...
      <Panel className="overflow-hidden">
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4 sm:px-6"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-yellow-400">Correct Answers</p><h3 className="mt-1 font-display text-xl font-black uppercase text-white">Seven-Point Result Sheet</h3></div><DemoLabel /></div>
        <div className="divide-y divide-white/10">
          {dutchGrandPrixQuestions.map((question) => {
            const options = question.type === 'Driver' ? driverOptions : constructorOptions;
            const value = answers[question.id] ?? '';
            return (
              <div key={question.id} className="grid gap-4 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_minmax(240px,0.55fr)_auto] lg:items-center">
                <div><div className="flex items-center gap-2"><span className="font-display text-sm font-black text-yellow-300">Q{question.id}</span><code className="text-[10px] text-zinc-600">{question.key}</code></div><p className="mt-2 text-sm font-semibold leading-6 text-white">{question.text}</p><p className="mt-1 text-xs text-zinc-500">1 point per correct answer</p></div>
                <label className="block"><span className="sr-only">Correct answer for question {question.id}</span><select value={value} onChange={(event) => setAnswers((current) => ({ ...current, [question.id]: event.target.value }))} className={inputClass}><option value="">Select correct answer</option>{options.map((option) => <option key={option}>{option}</option>)}</select></label>
                <span className={`inline-flex w-fit items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.13em] ${value ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-300' : 'border-white/10 bg-white/5 text-zinc-500'}`}>{value && <CheckCircle2 className="h-3 w-3" aria-hidden="true" />}{value ? 'Correct Answer Ready' : 'Awaiting Answer'}</span>
              </div>
            );
          })}
        </div>
      ...
      {reviewOpen && <Panel className="p-5 sm:p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-yellow-400">Review Summary</p><p className="mt-2 text-sm text-zinc-300">{answeredCount} of 7 correct answers are selected. This review is local demo state only.</p></div><CheckCircle2 className="h-6 w-6 text-yellow-400" aria-hidden="true" /></div></Panel>}
      ...
      <Modal open={confirmationOpen} onClose={() => setConfirmationOpen(false)} title="Calculate Demo Scores?" description="Confirmation is required before previewing the scoring state.">
        <p className="text-sm leading-6 text-zinc-300">A production workflow would compare 94 submissions with these seven correct answers. This prototype will not call an RPC or write any score.</p>
